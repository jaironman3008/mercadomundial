...
class classVenta{
	...
	public function updatecobros($idcobro,$mora,$sw){		
		if(Conectar::con()==true){
			switch($sw){
				case 'pagototal':
					$query="update cobros set estado='pagototal', fechapago=now() where idcobro='$idcobro'";break;				
				default:
					$query="update cobros set estado='cancelada', fechapago=now() where idcobro='$idcobro'";break;
			}			
	...
	public function updateventa($codigo,$sw,$obser){	
		if(Conectar::con()==true){			
			switch($sw){
				case '':	$query="update ventas set estado='cancelada' where idventa='$codigo'";break;
				case 'sindeuda':
							...
							$result1=mysql_query($query1);$t1=mysql_result($result1,0);
							$result2=@mysql_query($query2);$t2=mysql_result($result2,0);
							$result3=mysql_query($query3);$t3=mysql_result($result3,0);				
							$res=$t1+$t2+$t3;							
							$query="update ventas set estado='cancelada', montodeventa='$res' where idventa='$codigo'";break;
				case 'enproceso':	$query="update ventas set estado='en proceso' where idventa='$codigo'";break;
				case 'enmora':	$query="update ventas set estado='pendiente' where idventa='$codigo'";break;
				case 'revertido':	
								...
								$query3="update cobros set estado='congelado' where idventa='$codigo' and estado='pendiente'";break;
				case 'reventa':	$query="update ventas set estado='cancelada', observacion='complemento de la venta $obser' where idventa='$codigo'";break;
			}
			if($sw=='revertido'){
	...
	public function selectMaxid($sw){
		if(Conectar::con()==true){
			switch($sw){
				case 'recibo': 	$query="select max(idrecibo) as maxidrecibo from recibos";break;										  
				case 'venta':	$query = "select max(idventa) as idvent from ventas";break;
				case 'garante':	$query = "select max(idgarante) as maxidgarante from garantes";break;
			}
			$result=mysql_query($query);
	...
	public function misclientes($usuario,$from,$desde,$hasta){
		if(Conectar::con()==true){
			...
			if($usuario=='')$swuser='';
			if($desde=='' || $hasta=='')$rangofecha='';			
		
			switch($from){
				case 'todos':	$query="select v.idventa,c.idcliente,c.nombre,c.appaterno,c.apmaterno, c.ci,c.telefonocel,c.smspromocion,c.smsmora,c.smscredito, count(idventa) as nventas,u.nombre as nameuser,u.appaterno as appuser,u.apmaterno as apmuser
				...
				case 'casimora':	$query="select v.idventa,c.idcliente, c.nombre,c.appaterno,c.apmaterno,c.ci,c.telefonocel,c.smspromocion,c.smsmora,c.smscredito,u.nombre as nameuser,u.appaterno as appuser,u.apmaterno as apmuser,co.fechavcto
				...
				case 'enmora': $query="select v.idventa,c.idcliente, c.nombre,c.appaterno,c.apmaterno,c.ci,c.telefonocel,c.smspromocion,c.smsmora,c.smscredito,u.nombre as nameuser,u.appaterno as appuser,u.apmaterno as apmuser,co.fechavcto
										from cobros co, clientes c,ventas v, usuarios u
										where co.idventa=v.idventa and
										v.idcliente=c.idcliente and
										v.idusuario=u.idusuario  $swuser and
				...
				case 'enproceso':	$query="select v.idventa,c.idcliente, c.nombre,c.appaterno,c.apmaterno,c.ci,c.telefonocel,u.nombre as nameuser,u.appaterno as appuser,u.apmaterno as apmuser,co.fechavcto
											from cobros co, clientes c,ventas v, usuarios u
											where co.idventa=v.idventa and
											v.idcliente=c.idcliente and
											v.idusuario=u.idusuario  $swuser and
											co.interesmora>0 and
											co.estado='pendiente' and
											v.estado ='en proceso' group by idcliente";break;
				case 'revertido':	$query="select v.idventa,c.idcliente,c.nombre, c.appaterno, c.apmaterno,c.ci,c.telefonocel,u.nombre as nameuser,u.appaterno as appuser,u.apmaterno as apmuser
											from clientes c, ventas v, usuarios u
											where c.idcliente=v.idcliente and
				...
				case 'porcobrar':	$query="select c.idcliente, c.nombre,c.appaterno,c.apmaterno,c.ci,c.telefonocel,u.nombre as nameuser,u.appaterno as appuser,u.apmaterno as apmuser
											...
											co.estado='pendiente' and
											v.estado!='en proceso'";break;
				case 'todas':	$query="select v.idventa,v.fechaventa,c.nombre,c.appaterno,c.apmaterno,g.descripcion,v.montodeventa
										...
										p.idgrupo=g.idgrupo and
										dv.idproducto=p.idproducto order by idventa";break;
				case 'alcredito':		$query="select v.idventa,v.fechaventa,c.nombre,c.appaterno,c.apmaterno,g.descripcion,v.montodeventa
										from ventas v, usuarios u,clientes c, detalleventa dv, productos p, grupos g 
										where v.idusuario=u.idusuario $swuser and										
										$rangofecha
										v.idcliente=c.idcliente and
										...
										p.idgrupo=g.idgrupo and
										dv.idproducto=p.idproducto and
										tipoventa='credito' order by idventa";break;
				case 'alcontado':		$query="select v.idventa,v.fechaventa,c.nombre,c.appaterno,c.apmaterno,g.descripcion,v.montodeventa
										from ventas v, usuarios u,clientes c, detalleventa dv, productos p, grupos g 
										where v.idusuario=u.idusuario $swuser and										
										$rangofecha
										v.idcliente=c.idcliente and
	...
	public function clienteyventa($idcliente,$from){
		if(Conectar::con()==true){
			switch($from){
				case 'todos':$query="select idventa,c.nombre,c.appaterno,c.apmaterno, montodeventa, cuotamensual, cuotainicial, nrocuotas, fechainicial, fechaventa,horaventa, tipoventa,estado 
									from clientes c, ventas v
									where c.idcliente=v.idcliente and
									c.idcliente='$idcliente'";break;
				case 'casimora':	$query="select v.idventa,c.nombre,c.appaterno,c.apmaterno, montodeventa, cuotamensual, cuotainicial, nrocuotas, fechainicial, fechaventa,horaventa, tipoventa,v.estado 
											...
											co.estado='pendiente' and
											v.estado!='en proceso' and
											co.fechavcto<= curdate()";break;
				case 'enmora':	$query="select v.idventa,c.nombre,c.appaterno,c.apmaterno, montodeventa, cuotamensual, cuotainicial, nrocuotas, fechainicial, fechaventa,horaventa, tipoventa,v.estado 
										from clientes c, ventas v, cobros co
										where c.idcliente=v.idcliente and
										c.idcliente='$idcliente' and
										v.idventa=co.idventa and
										...
										co.estado='pendiente' and
										v.estado!='en proceso'";break;
				case 'enproceso':	$query="select v.idventa,c.nombre,c.appaterno,c.apmaterno, montodeventa, cuotamensual, cuotainicial, nrocuotas, fechainicial, fechaventa,horaventa, tipoventa,v.estado 
											from clientes c, ventas v, cobros co
											where c.idcliente=v.idcliente and
											c.idcliente='$idcliente' and
											v.idventa=co.idventa and
											...
											co.estado='pendiente' and
											v.estado='en proceso'";break;
				case 'revertido':	$query="select v.observacion
											from clientes c, ventas v
											...
											v.idcliente=c.idcliente and
											c.idcliente='$idcliente' and
											v.estado='revertido'";break;
				case 'porcobrar':  $query="select v.idventa,c.nombre,c.appaterno,c.apmaterno, montodeventa, cuotamensual, cuotainicial, nrocuotas, fechainicial, fechaventa,horaventa, tipoventa,v.estado 
											from clientes c, ventas v, cobros co
											where c.idcliente=v.idcliente and
											c.idcliente='$idcliente' and
											v.idventa=co.idventa and
											...
											co.estado='pendiente' and
											v.estado!='en proceso'";
				case 'pendiente':$query="select idventa,c.nombre,c.appaterno,c.apmaterno, montodeventa, cuotamensual, cuotainicial, nrocuotas, fechainicial, fechaventa,horaventa, tipoventa,estado 
									from clientes c, ventas v
									where c.idcliente=v.idcliente and
	...
	public function calcularprecio($precio,$idproducto){
		if(Conectar::con()==true){
			...
			$result1=mysql_query($query1);$idventa=mysql_result($result1,0);
			...
			$query3="select sum(interesmora) as morasinpagar from cobros c
					where idventa='$idventa' and
					c.estado='congelado'";				
			$result2=mysql_query($query2);$congeladosinpagar=mysql_result($result2,0);
			$result3=mysql_query($query3);$morasinpagar=mysql_result($result3,0);
			$query4="select sum(cuota) as cobrosinpagar from cobros c
					where idventa='$idventa'";
			$result4=mysql_query($query4);$cobrosinpagar=mysql_result($result4,0);
			$query5="select (($congeladosinpagar*100)/$cobrosinpagar) as porsinpagar";			
			$result5=mysql_query($query5);$porsinpagar=mysql_result($result5,0);
			$query6="select '$morasinpagar'+(('$porsinpagar'/100)*'$precio') as precio";
			$result6=mysql_query($query6);$precio=mysql_result($result6,0);
	...
	/***********************************************************DISTRIBUIR LAS FECHAS ENTRE LOS MESES*/
