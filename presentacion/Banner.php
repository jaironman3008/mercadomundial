...
class Banner{
		...
		$slider="<div id='wowslider-container1'>
					<div class='ws_images'>
						...
						for($i=0;$i<count($lista);$i++){
							...
							//else
								//$slider.="<li><img width='960px' height='360px' src='images/depositos/".$depositoImg[0]['depositoImg']."' title='Mi Deposito'/></li>";
						}
			...
			$slider.="</ul>
					...
					<div class='ws_bullets'>
						...
						for($i=0;$i<count($lista);$i++){
							...
							//else
							//	$slider.="<a title='Mi Deposito'><img width='128' height='48' src='images/depositos/".$depositoImg[0]['depositoImg']."'/></a>";
						}
						//var_dump($lista);
						//$slider.="<a title='Mi Deposito'><img src='images/depositos/".$depositoImg[0]['depositoImg']."'/></a>";						
						$slider.="	
						</div>
					</div>					
				</div>
	...
	public function getImgBanner($sw='todos'){
			...
			switch($sw){
				case 'si': $query="select * from banners where ver='si'";break;
				case 'no': $query="select * from banners where ver='no'";break;
				default: $query="select * from banners";
			}			
	...
	public function redimensionarImagen($ruta,$filename){
		...
		$rtOriginal=$dir;
		switch($extencion){
			case 'jpg':$original = imagecreatefromjpeg($rtOriginal);break;
			case 'jpeg':$original = imagecreatefromjpeg($rtOriginal);break;
			case 'png':$original = imagecreatefrompng($rtOriginal);break;			
		}
		...
		switch($extencion){
			case 'jpg':imagejpeg($lienzo,"../images/banner/tooltips/".$filename,$cal);break;
			case 'jpeg':imagejpeg($lienzo,"../images/banner/tooltips/".$filename,$cal);break;
			default: imagepng($lienzo,"../images/banner/tooltips/".$filename,9);
		}		
	}
	...
	public function actualizarBanners($idBanner,$verBanner){
		...
		if($verBanner=='si')$ver='no';else $ver='si';
