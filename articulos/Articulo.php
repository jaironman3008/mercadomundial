...
class Articulo extends DBManager
{
	...
	{
		$query = "SELECT * from articulos where idusuario=" . $this->mySql->real_escape_string($userId) . " and revisado='aceptado'";
		$result = $this->mySql->query($query);
		if (!$result)
			return false;
		else
	...
	{
		//same function in class Usuario
